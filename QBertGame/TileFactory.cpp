...
std::shared_ptr<GameObject> TileFactory::MakeTile(const std::vector<std::string>& textures, float posX, float posY, float size, 
	int row, int col, bool isCyclical)
{
	auto go = std::make_shared<GameObject>();
	go->AddComponent(new TransformComponent(posX, posY));
	go->AddComponent(new TextureComponent(textures[0], size, size));
	go->AddComponent(new TileComponent(textures, col, row, isCyclical));
